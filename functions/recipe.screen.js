...
function searchRecipeIntent(conv, params) {
  ...
  return database.search({ term: params.recipe, page }).then(data => {
    ...
    if (data.total < 1) {
      ...
      const newName = (name, list, last) => {
        let fixedName = name;
        if(last && last > 0) {
          fixedName = `${name} ${last + 1}`;
        } 
        if(list.includes(fixedName)) {
          return newName(name, list, ++last);
        } else {
          return fixedName;
        }
      }
      ...
      const items = data.recipes.reduce((acc, recipe) => {
        acc[recipe.id] = {
          title: newName(recipe.name, Object.values(acc).map(a => a.title), 0),
